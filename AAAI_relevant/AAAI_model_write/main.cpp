...
const string node_file = "7node.txt";
const float final_point = 0.61;
const float first_point = 0.6;

float frac = first_point;
const int subsidyNum = 0;
float taxW =3;
...
//	outfile.close();
	...
	while(frac<final_point)
    {
        ...
        ne.init();
        outfilemin<<t<<"\t"<<ne.HDG(t)<<endl;
//        get_degree_info(ne);
        //ne.init();
        //outfilemax<<t<<"\t"<<maxNE(ne,t)<<endl;
