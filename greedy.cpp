...
double GetCost(vector<CNode> &nodeVec, vector<CLink> temp_linkVec, deque<size_t> greedyResult)
{
    ...
    BrushMinDistance(nodeVec, temp_linkVec);                //初始时的最短路径
    ...
    while(currentDist < g_allDist)
    {
        if(currentDist == change)
        {
            ...
            BrushMinDistance(nodeVec, temp_linkVec);
        ...
        for(auto i = nodeVec.begin(); i!= nodeVec.end(); i++)
        {
            logistCost[index] += (i->m_cargoNum / (g_allDist/10)) * i->m_minDistance * 10 * 1000;
            i->m_logistCost = logistCost[index];
            index++;
        }
        currentDist += 1;
    }
//    cout<<logistCost[0]<<endl;
//    cout<<nodeVec[0].m_logistCost<<endl;
...
deque<size_t> Greedy(vector<CNode> nodeVec, vector<CLink> linkVec)
{
    ...
    for(size_t i=0; i<sum; i++)
    {
        ...
        sort(cost.begin(), cost.end(),
             [](const pair<size_t, double> &s1, const pair<size_t, double> &s2){return s1.second < s2.second;});
